...
class _BookmarkedDocumentViewScreenState
  ...
  void initState() {
    ...
    // viewSearchedIndexedDocument(widget.fileNames[fileIndex]['documentId'], fileIndex);
...
// -------- GET API - get selected document to view -------------//
  ...
  Future<void> addAccessTrackOperation() async {
    var response = await ApiService.passAccessTrackAction(
      documentId.toString(),
      username,
      operation,
      templaetName,
  ...
  Future<void> getCompletedTasks() async {
    var response4 = await ApiService.getCompletedTaskByDocId(documentId, token);
  ...
  Future<void> getSelectedPendingTaskByDocId() async {
    var response5 = await ApiService.getSelectedPendingTaskByDocId(
        username, documentId, token);
    if (response5 != null && response5.statusCode == 200) {
      responsedata = jsonDecode(response5.body);

      if (mounted)
        setState(() {
          pendingTaskById = responsedata!['value'];
        });
    } else if (response5.statusCode == 500) {
      showWarningDialogPopup(
        context,
        Icons.warning,
        "Server error! Please contact auraDOCS administrator.",
